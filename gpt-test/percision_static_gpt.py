...
if __name__ == "__main__":
    ...
    team_folders = ["member1_gpt", "member2_gpt"]
    # get time now
    current_time = "percision_gpt"#time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    info_term = [[],[]]
    info_sentence = [[],[]]
    bert_replace = [[],[]]
    # realistic = [[],[]]
    ...
    for i in range(len(mr_types)):
        ...
        for j in range(len(mt_systems)):
            ...
            for k, row in data1.iterrows():
                ...
                if row.values[-1] == "有错误" or row.values[-1] == 1:
                    manual_1_result = 1
                elif row.values[-1] == "无错误" or row.values[-1] == 0:
                    manual_1_result = 0
                else:
    ...
    bert_replace_kappa = compute_kappa_score(general_labels[2][0][200:], general_labels[2][1][200:])
