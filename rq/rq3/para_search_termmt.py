...
def main():
    file_floder = "./results"
    systems = ["bing", "google", "mbart"]
    ques_time = "statistics-termmt"
    ...
    for system in systems:
        system_label_dict[system] = []
        file_path = f"{file_floder}/{ques_time}/{system}_conflict_solved.json"
        ...
        for key in data.keys():
            ...
            system_label_dict[system].append([index, item["insert和原句术语翻译相似度"][0], item["insert和原句翻译去括号部分相似度"][0], max(item["替换术语和原句术语翻译相似度"])])
        ...
        s=f"system: {system}, insert_sentence_error: {insert_sentence_error}, insert_term_error: {insert_term_error}, replace_term_error: {replace_term_error}, general_error: {general_error}, effective_count: {len(data)}"
    ...
    for system in systems:
        ...
        replace_term_error = 0
        general_error = 0
        for item in system_label_dict[system]:
            ...
            if item[1] <= bst_insert_term_similarity or item[2] <= bst_insert_sentence_similarity or item[3] >= bst_bert_term_similarity:
                general_error += 1
        result_strs.append(f"system: {system}, insert_sentence_error: {insert_sentence_error}, insert_term_error: {insert_term_error}, replace_term_error: {replace_term_error}, general_error: {general_error}")
