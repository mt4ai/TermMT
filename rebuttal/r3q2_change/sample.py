...
if __name__ == "__main__":
    ...
    for i in range(len(area_folders)):
        ...
        json__error_google = read_json(f"{area_folder}/error_google_sbert/metamorphic_item_error_gptfiltered.json")
        
        jsons_error_google.extend(json__error_google)
    
    sampled_errors = rand_sample(jsons_error_google, 100, 1234)

    sampled_infinsert_term_errors = []
    sampled_infinsert_sentence_errors = []
    sampled_bert_replace_errors = []

    for error in sampled_errors:
        if error["phrase_infinsert_error_term"] == 1:
            sampled_infinsert_term_errors.append(error)
        if error["phrase_infinsert_error_sentence"] == 1:
            sampled_infinsert_sentence_errors.append(error)
        if error["phrase_bertinsert_error_term"] == 1:
            sampled_bert_replace_errors.append(error)

    json_to_csv_insert_term_error(sampled_infinsert_term_errors, os.path.join(output_path, "info_term_error_google.csv"))
    json_to_csv_insert_sentence_error(sampled_infinsert_sentence_errors, os.path.join(output_path, "info_sentence_error_google.csv"))
    json_to_csv_bert_replace_error(sampled_bert_replace_errors, os.path.join(output_path, "bert_replace_error_google.csv"))
    write_json(sampled_infinsert_term_errors, os.path.join(output_path, "info_term_error_google_sampled.json"))
    write_json(sampled_infinsert_sentence_errors, os.path.join(output_path, "info_sentence_error_google_sampled.json"))
    write_json(sampled_bert_replace_errors, os.path.join(output_path, "bert_replace_error_google_sampled.json"))
