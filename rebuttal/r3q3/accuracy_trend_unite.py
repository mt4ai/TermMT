...
def draw_f1_accuracy_recall(f1_lst, accuracy_lst, recall_lst, count_errors , parameters_lst, pic_name, pic_path, legend_name, xbg, xed):
    plt.figure()
    ...
    ax1.set_xlabel('Thesholds for '+legend_name, fontsize=font_size)
    ax1.set_ylabel('Score', fontsize=font_size)

    # 创建第二个子图，并共享x轴
    y_paddding = 1.01
    x_padding = 1.01
    ax1.set_xlim((xbg-0.1)*x_padding, xed*x_padding)
    ax2 = ax1.twinx()
...
def search_best_parameters(pred_similaritys, refer_labels, file_floder, pic_name, bg_similarity, ed_similarity, step, legend_name, type="no smaller than"):
    ...
    xbg = min(pred_similaritys)
    xed = max(pred_similaritys)
    ...
    draw_f1_accuracy_recall(f1_lst, accuracy_lst, recall_lst, count_errors_lst, similarities, pic_name, f"{file_floder}/{pic_name}.png", legend_name, xbg, xed)
...
def main():
    ...
    unite_data = read_json("./results/unite_scores.json")
    ...
    for i in range(len(datas)):
        ...
        model = models[i]
        for item in data:
            pred_similaritys.append(item["Score"])
            if item["Type"] == "Error":
        ...
        ed_similarity = 1
        step = 0.01
        bst_metrics = search_best_parameters(pred_similaritys, refer_labels, f"./pics", f"Accuracy_trend_{model}", bg_similarity, ed_similarity, step, legend_names[i])

        # write into txt
        with open(f"./results/best_parameters_{model}.txt", "w") as f:
